...
courseRouter.post(
    "/courses",
    authenticate(),
    uploadImage.single("cover"), // the cover of course's video
    validateBody(CourseDto),
    async (req, res) => {
        const repo = AppDataSource.getRepository(Course);
...
courseRouter.put(
    ...
    async (req, res, next) => {
        try {
            ...
            if (!course) return next(new ErrorException(404, "Course not found"));
            ...
            if (req.body.authorId !== undefined) course.authorId = Number(req.body.authorId);
            if (req.body.categoryId !== undefined) course.categoryId = Number(req.body.categoryId);
            if (req.body.difficultyId !== undefined) course.difficultyId = Number(req.body.difficultyId);
            if (req.body.languageId !== undefined) course.languageId = Number(req.body.languageId);
