 ...
 *       - bearerAuth: []
 */
courseReviewRouter.post("/courses/:courseId/reviews", authenticate(), async (req, res, next) => {
    try {
        // @ts-ignore
 ...
 *       - bearerAuth: []
 */
courseReviewRouter.delete("/course-reviews/:id", authenticate(), async (req, res, next) => {
    try {
        const id = Number(req.params.id);
        const repo = AppDataSource.getRepository(CourseReview);
        const review = await repo.findOneBy({ id });
        if (!review)
            return next(new ErrorException(404, "Review not found"));
